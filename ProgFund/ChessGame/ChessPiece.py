...
class GeneralStrategy(MoveStrategy):
    def movement(self, current_pos, target_pos, board, current_player):
        ...
        if current_player == 'r':
            return ((3 <= target_x <= 5 and 7 <= target_y <= 9) and
                    abs(current_x - target_x) + abs(current_y - target_y) == 1)
        else:
            return ((3 <= target_x <= 5 and 0 <= target_y <= 2) and
                    abs(current_x - target_x) + abs(current_y - target_y) == 1)
...
class AdvisorStrategy(MoveStrategy):
    def movement(self, current_pos, target_pos, board, current_player):
        ...
        if current_player == 'r':
            return ((3 <= target_x <= 5 and 7 <= target_y <= 9) and
                    abs(current_y - target_y) == 1 and abs(current_x - target_x) == 1)
        else:
            return ((3 <= target_x <= 5 and 0 <= target_y <= 2) and
                    abs(current_y - target_y) == 1 and abs(current_x - target_x) == 1)
...
class ElephantStrategy(MoveStrategy):
    def movement(self, current_pos, target_pos, board, current_player):
        ...
        target_x, target_y = target_pos
        if current_player == 'r':
            return (target_y >= 5 and abs(current_x - target_x) == 2 and abs(current_y - target_y) == 2 and
                    board[(current_y + target_y) // 2][(current_x + target_x) // 2] is None)
        else:
            return (target_y <= 4 and abs(current_x - target_x) == 2 and abs(current_y - target_y) == 2 and
                    board[(current_y + target_y) // 2][(current_x + target_x) // 2] is None)
    ...
    def movement(self, current_pos, target_pos, board, current_player):
        ...
        if current_player == 'r':
            return ((target_x == current_x and target_y == current_y - 1) or
                    (abs(target_x - current_x) == 1 and current_y == target_y and current_y <= 4))
        else:
            return ((target_x == current_x and target_y == current_y + 1) or
                    (abs(target_x - current_x) == 1 and current_y == target_y and current_y >= 5))
