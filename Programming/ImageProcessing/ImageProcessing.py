...
class ImageProcessing:
    ...
    def __init__(self, path: str = None):
        ...
        if path is not None:
            ...
            # 归一化到 uint8
            if (
                self.image_orig.dtype == np.float32
                or self.image_orig.dtype == np.float64
            ):
    ...
    def _interpolate(self, src, x, y, dst):
        ...
        dst[:] = (
            wa * src[y0, x0] + wb * src[y0, x1] + wc * src[y1, x0] + wd * src[y1, x1]
        ).astype(np.uint8)
...
class Window:
    ...
    def _setup_ui(self):
        ...
        tk.Button(row1, text="选择图片", command=self.load_image).pack(
            side=tk.LEFT, padx=5
        )
        tk.Radiobutton(
            row1,
            text="彩色图",
            variable=self.use_gray,
            value=False,
        ...
        self.transform_combo = ttk.Combobox(
            row2, textvariable=self.transform_var, state="readonly", width=20
        )
        ...
        tk.Button(row2, text="确认变换", command=self.apply_single_transform).pack(
        ...
        tk.Button(row3, text="复合变换", command=self.open_composite_dialog).pack(
            side=tk.LEFT, padx=10
        )
        tk.Button(row3, text="SVD压缩", command=self.open_svd_dialog).pack(
            side=tk.LEFT, padx=10
        )
        tk.Button(row3, text="重置变换", command=self.reset_transform).pack(
            side=tk.LEFT, padx=10
        )
    ...
    def _update_display(self):
        ...
        # 根据模式选择原始图像
        original = (
            self.img_proc.image_gray
            if self.use_gray.get()
            else self.img_proc.image_orig
        )
        # 应用累积变换矩阵
        transformed, extent = self.img_proc._apply_transform(
            original, self.total_matrix, is_homogeneous=True
        )
    ...
    def open_composite_dialog(self):
        ...
        # 界面布局
        tk.Label(dialog, text="第一/二类变换:").grid(
        ...
        tk.Button(dialog, text="确认", command=apply_composite).grid(
    ...
    def open_svd_dialog(self):
        ...
        current_img, _ = self.img_proc._apply_transform(
            original, self.total_matrix, is_homogeneous=True
        )
        ...
        scale = tk.Scale(
            dialog, from_=1, to=max_k, orient=tk.HORIZONTAL, variable=k_var, length=300
        )
        scale.pack()
        ...
        def apply_svd():
            ...
            if len(current_img.shape) == 2:
                U, s, Vt = np.linalg.svd(
                    current_img.astype(np.float64), full_matrices=False
                )
                S = np.diag(s[:k])
