...
class DataAnalysis:
    def __init__(self, path: str):
        ...
        self.data = pd.read_csv(
            os.path.join(path, "lagou_data.csv"), encoding="gbk"
        ).reset_index()
    ...
    def missing_process(self):
        ...
        missing_stats = pd.DataFrame(
            {
                "缺失值数量": self.data.isnull().sum(),
                "缺失值比例": (
                    self.data.isnull().sum() / self.data.shape[0] * 100
                ).round(2),
            }
        )
        print("缺失数据：\n", missing_stats)
    ...
    def feature_extraction(self):
        ...
        catgorical_cols = self.data.select_dtypes(
        ...
        if numerical_cols:
            ...
            for col in numerical_cols:
                ...
                oulier_ratio = (
                    len(ouliers) / self.data.shape[0] * 100 if len(ouliers) else 0
                )
    ...
    def mutual_info_analysis(self, k=10):
        ...
        if "salary" in self.data.columns:
            ...
            salary_col = self.data.columns[
                self.data.columns.str.contains("salary|薪资")
            ][0]
        ...
        # 合并特征矩阵
        ordinal_encoded = self.data[
            [col + "_encoded" for col in ordinal_features]
        ].values
