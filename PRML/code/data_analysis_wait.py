...
class DataAnalysis:
    ...
    def classify_salary(self, salary):
        """内部使用的薪资分类函数"""
        if salary < 15000:
            return '15k以下'
        elif salary <= 25000:
    ...
    def process_and_plot_topk_mutual_info(self, k=20):
        """
        1. 处理薪资数据 (调用 classify_salary)
        2. 处理特征 (Label Encoding)
        3. 计算互信息并绘制 Top-K 相关性热力图
        """
        print('\n' + '=' * 50)
        print("开始处理 Top-K 工资水平影响力互信息图...")

        # 1. 确保数据已清洗
        if self.df is None:
            self.missing_process() # 假设清洗后的数据存放在 self.df

        df_clean = self.df.copy()

        # 2. 应用薪资分类 (统一薪资数据)
        df_clean['salary_range'] = df_clean['salary'].apply(self.classify_salary)
        
        # 3. 特征工程：选择需要分析的特征 (互信息需要数值型输入)
        # 选择你认为可能影响薪资的特征 (Ordinal Features)
        categorical_features = ['city', 'education', 'work_year', 'company_name', 'position_name']
        # 过滤掉不存在的列
        available_features = [col for col in categorical_features if col in df_clean.columns]
        
        # 4. 编码：将文本特征转换为数值 (互信息计算需要)
        le = LabelEncoder()
        encoded_data = {}
        ...
        for col in available_features:
            ...
            encoded_data[col] = le.fit_transform(df_clean[col])
        
        # 将薪资区间也转换为数值标签 (作为目标变量 y)
        df_clean['salary_range_encoded'] = le.fit_transform(df_clean['salary_range'])
        
        # 5. 构建用于分析的数据框
        feature_matrix = pd.DataFrame(encoded_data)
        target_vector = df_clean['salary_range_encoded']

        # 6. 计算互信息 (Mutual Information)
        # 互信息衡量的是特征包含关于目标变量的信息量
        mi_scores = mutual_info_regression(feature_matrix, target_vector, random_state=42)
        mi_scores = pd.Series(mi_scores, name="MI Scores", index=feature_matrix.columns)
        mi_scores = mi_scores.sort_values(ascending=False)

        print("特征与薪资水平的互信息得分 (Top-K):")
        print(mi_scores.head(k))

        # 7. 绘制 Top-K 互信息条形图 (展示哪些因素对薪资影响力最大)
        plt.figure(figsize=(10, 6))
        top_k_features = mi_scores.head(k).index
        sns.barplot(x=mi_scores.head(k).values, y=top_k_features, palette="viridis")
        plt.title(f'Top {k} 特征对薪资水平 (Salary Range) 的影响力 (互信息)')
        plt.xlabel('互信息得分 (Mutual Information Score)')
        plt.ylabel('特征名称')
        plt.show()

        # 8. (可选) 如果你想看特征之间的互信息热力图 (Correlation-like)
        # 由于互信息矩阵计算较复杂，通常我们看上述的特征-目标得分。
        # 如果你需要特征之间的两两互信息，可以使用 seaborn 的 clustermap
        # 这里简单展示前几个特征的互信息热力图
        if len(feature_matrix.columns) > 1:
            # 计算两两特征之间的互信息 (模拟相关性矩阵)
            n_features = len(feature_matrix.columns)
            mi_matrix = np.zeros((n_features, n_features))
            
            for i, col1 in enumerate(feature_matrix.columns):
                for j, col2 in enumerate(feature_matrix.columns):
                    if i == j:
                        mi_matrix[i, j] = 1.0 # 自身相关性设为1
                    else:
                        # 计算互信息
                        mi = mutual_info_regression(feature_matrix[[col1]], feature_matrix[col2].ravel(), random_state=42)[0]
                        mi_matrix[i, j] = mi
            
            # 绘制热力图
            plt.figure(figsize=(8, 6))
            sns.heatmap(mi_matrix, 
                        xticklabels=feature_matrix.columns, 
                        yticklabels=feature_matrix.columns, 
                        annot=True, cmap='Blues', fmt='.2f')
            plt.title(f'特征间互信息热力图 (Top-K)')
            plt.show()

        # 保存处理后的数据
        df_clean.to_csv(os.path.join(self.path, 'lagou_processed_for_mi.csv'), index=False, encoding='utf-8')
        print(f"\n处理完成。清洗后的数据已保存，包含薪资分类特征。")
...
if __name__ == "__main__":
    ...
    anlys.process_and_plot_topk_mutual_info(k=10)
    ...
    # # 生成薪资区间（自动适配薪资列名）
    # salary_col = 'salary' if 'salary' in df_clean.columns else \
    # df_clean.columns[df_clean.columns.str.contains('salary|薪资')][0]
    # df_clean['salary_range'] = df_clean[salary_col].apply(classify_salary)
    # print(f"✅ 新增薪资区间段'salary_range'，取值分布:")
    # print(df_clean['salary_range'].value_counts())

    # # ===================== 5. 特征编码（有序+无序） =====================
    # # 5.1 有序特征（LabelEncoder）
    # ordinal_features = ['work_year', 'education', 'size_standard']
    # ordinal_features = [f for f in ordinal_features if f in df_clean.columns]  # 只保留存在的列
    ...
    # nominal_features = [f for f in nominal_features if f in df_clean.columns]  # 只保留存在的列
    # onehot = OneHotEncoder(sparse_output=False, drop='first')
    # nominal_encoded = onehot.fit_transform(df_clean[nominal_features])

    # # 5.3 合并特征矩阵
    # ordinal_encoded = df_clean[[col + '_encoded' for col in ordinal_features]].values
    # X = np.hstack([nominal_encoded, ordinal_encoded])  # 特征矩阵
    # y = LabelEncoder().fit_transform(df_clean['salary_range'])  # 目标变量（薪资区间）
