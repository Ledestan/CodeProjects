...
if __name__ == "__main__":
    ...
    print("请选择要运行的任务：")
    print("  0 - IrisDimReduction")
    print("  1 - FaceDimReduction")
    print("  2 - FaceRecAnalysis")
    ...
    if task == "IrisDimReduction":
        pca = IrisDimReduction()
        pca.data_processing()
        pca.show_ratio()

    elif task == "FaceDimReduction":
        pca = FaceDimReduction(data_path, (64, 64))
        pca.load_face_data()
        pca.data_processing()
        pca.show_PCA()
    ...
    elif task == "FaceRecAnalysis":
        # 初始化并加载数据
        FaceRec = FaceRecAnalysis(data_path, (64, 64))
        FaceRec.load_data()
        ...
        try:
            # 测试 1: 同一个人
            is_same_A, sim_A = verifier.verify(
                f"{data_path}/s1/1.pgm", f"{data_path}/s1/2.pgm"
            )
            print(
                f"测试A - 同一个人: {'是' if is_same_A else '否'} (相似度: {sim_A:.4f})"
            )

            # 测试 2: 不同的人
            is_same_B, sim_B = verifier.verify(
                f"{data_path}/s1/1.pgm", f"{data_path}/s2/1.pgm"
            )
            print(
                f"测试B - 不同的人: {'是' if is_same_B else '否'} (相似度: {sim_B:.4f})"
            )
